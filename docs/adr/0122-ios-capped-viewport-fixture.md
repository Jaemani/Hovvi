...
`capped-viewport` preview fixture sets the cap to 8 rows, and
`TerminalSurfaceProjection.viewport(for:)` uses this snapshot cap when callers do
not provide an explicit limit.
